 ...
 * 
 * SERVER-ONLY: This module should only be used in API routes
 */

// Ensure this is server-side only
if (typeof window !== 'undefined') {
  throw new Error('Storage module can only be used server-side')
}
...
async function saveConfig(config) {
    ...
    if (isVercel && kvClient) {
        try {
            ...
            // Don't throw immediately - try to provide helpful error
            throw new Error('Vercel KV not available. Please create a KV database in Vercel Dashboard → Storage → Create Database → KV. Then redeploy your project.');
        }
    }
    ...
    // If we're on Vercel but KV isn't available
    if (isVercel) {
        throw new Error('Vercel KV not available. Please create a KV database in Vercel Dashboard → Storage → Create Database → KV. Then redeploy your project.');
    }
